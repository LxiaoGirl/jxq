...
class Member_model extends CI_Model
{
    ...
    const user        = 'user'; // 会员表
	const product     = 'product_category'; // 项目表
    ...
    public function updatea()
    {
        ...
        if( ! empty($temp['uid']))
        {
			...
			if($temp['group_id']==4){
				if($user['inviter_no']==0){
					$temp['data']['inviter_no'] = $this->c->transaction_no(self::user, 'inviter_no');
					$temp['data']['lv'] = "1";
				}
			}
    ...
    public function get_member_info($flag = TRUE)
    {
		...
		if( ! empty($temp['uid']))
		{
	    	...
	    	if( ! empty($data) && ! empty($flag))
	    	{
				...
				$data['allinvest'] = $this->_get_allinvest($temp['uid']);
	    	}
		}
		unset($temp);
		return $data;
    ...
    public function get_product_category_list()
    {
        ...
        $temp['data'] = $this->c->get_all(self::product, $temp['where']);
    ...
    public function automatic_update()
    {
        ...
        if( ! empty($temp['uid']))
        {
            ...
            $temp['data'] = array(			
                                ...
                                'type'    => $this->input->post('group_id', TRUE),//项目类型
                                'sy_min' => $this->input->post('sy_min', TRUE),//收益最小
                                'sy_max'      => $this->input->post('sy_max', TRUE),//收益最大
								'jk_min' => $this->input->post('jk_min', TRUE),//期限最小
								...
								'balance_ye'     => floor($balance*$pzed/10000)*100,//可用配置余额
                                'balance_ze'      => floor($balance*$pzed/10000)*100//配置总额
			);
