...
class Interest_model extends CI_Model
{
    ...
    public function processing($borrow_no='')
    {
	    ...
	    // 获取已还款记录
			...
			//var_dump($temp['borrow_start_list']);			
			$diff = array_diff($s,$t);
...
//            $today = date('Ymd');
    ...
    public function get_interest_list($borrow=array()){
        ...
        if( ! empty($borrow)){
            ...
            $temp['where'] = array(
                'select' => join_field('user_name,mobile', self::user) . ',' .
                    join_field('uid,payment_no,amount,borrow_no', self::payment).','.
...
//                    join_field('is_interest	', self::borrow) => 1 //支付利息中
            ...
            if (!empty($temp['data'])) {
                foreach ($temp['data'] as $k => $v) {
                    //获取支付记录
                    $temp['exist'] = $this->_get_payment_list($v['uid'], $v['borrow_no'],$v['exp_date']);
                    $v['is_pay']=0;
                    if ( ! empty($temp['exist'])) {
                        $v['is_pay']=1;
                    }
