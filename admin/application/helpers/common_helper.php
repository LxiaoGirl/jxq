 ...
 * 号分隔的字符串。
 * 数据格式如：array('admin/assets/css/style.css','admin/assets/js/common.js')
 *
 * @access public
...
{
    function display($user_data = '', $key = '', $default = '-')
    {
        if( ! empty($key))
        {
        ...
        return ( ! empty($user_data)) ? $user_data : $default;
    }
}
...
{
    function image($file = '', $width = 0, $height = 0)
    {
        $name = '';
    ...
    function load_file($files = '', $flag = FALSE)
    {
        ...
        if( ! empty($files))
        {
            ...
            foreach($temp['files'] as $k => $v)
            {
                ...
                {
                    $temp['data'][] = 'admin/assets/'.$temp['ext'].'/'.$v;
                }
            }
