...
//        }
    ...
    public function auth($options){
        ...
        if(!( ('/' == $options[self::OSS_OBJECT]) && ('' == $options[self::OSS_BUCKET]) && ('GET' == $options[self::OSS_METHOD])) && !$this->validate_bucket($options[self::OSS_BUCKET])){
            return array('data'=>'','info'=>'"'.$options[self::OSS_BUCKET].'"'.lang('OSS_BUCKET_NAME_INVALID'),'status'=>0);
        }

        //验证Object
        if(isset($options[self::OSS_OBJECT]) && !$this->validate_object($options[self::OSS_OBJECT])){
            return array('data'=>'','info'=>'"'.$options[self::OSS_OBJECT].'"'.lang('OSS_OBJECT_NAME_INVALID'),'status'=>0);
        }
        ...
        if(isset($options[self::OSS_HEADERS][self::OSS_ACL]) && !empty($options[self::OSS_HEADERS][self::OSS_ACL])){
            if(!in_array(strtolower($options[self::OSS_HEADERS][self::OSS_ACL]), self::$OSS_ACL_TYPES)){
                return array('data'=>'','info'=>$options[self::OSS_HEADERS][self::OSS_ACL].':'.lang('OSS_ACL_INVALID'),'status'=>0);
            }
        }
...
//            $hostname = (isset($options[self::OSS_BUCKET]) && ''!==$options[self::OSS_BUCKET])?$this->hostname.'/'.$options[self::OSS_BUCKET]:$this->hostname;
    ...
    public function list_bucket($options = array()) {
        ...
        if ($this->access_id == '' || $this->access_key == '') {
            return  array('data'=>'','info'=>lang('OSS_ACCESS_ID_OR_ACCESS_KEY_EMPTY'),'status'=>0);
        }

        //$options
        if ($options != NULL && ! is_array ( $options )) {
            return  array('data'=>'','info'=>lang('OSS_OPTIONS_MUST_BE_ARRAY'),'status'=>0);
        }
    ...
    public function create_bucket($bucket, $acl = 1, $options = array()){
        ...
        //bucket
        if(empty($bucket)){
            return  array('data'=>'','info'=>lang('OSS_BUCKET_IS_NOT_ALLOWED_EMPTY'),'status'=>0);
        }
    ...
    public function set_bucket_logging($bucket, $target_bucket, $target_prefix, $options = NULL){
        ...
        if(empty($target_bucket)){
            return  array('data'=>'','info'=>lang('OSS_TARGET_BUCKET_IS_NOT_ALLOWED_EMPTY'),'status'=>0);
        }
...
//        echo	$xml->asXML();
    ...
    public function set_bucket_website($bucket, $index_document, $error_document, $options = NULL){
        ...
        if(empty($index_document)){
            return  array('data'=>'','info'=>lang('OSS_INDEX_DOCUMENT_IS_NOT_ALLOWED_EMPTY'),'status'=>0);
        }
...
//        echo	$xml->asXML();
    ...
    public function create_object_dir($bucket, $object, $options = NULL){
        ...
        //object
        if(empty($object)){
            return  array('data'=>'','info'=>lang('OSS_OBJECT_IS_NOT_ALLOWED_EMPTY'),'status'=>0);
        }
    ...
    public function upload_file_by_content($bucket, $object, $options = NULL){
        ...
        if(isset($options[self::OSS_CONTENT])){
            if($options[self::OSS_CONTENT] == '' || !is_string($options[self::OSS_CONTENT])){
                return  array('data'=>'','info'=>lang('OSS_INVALID_HTTP_BODY_CONTENT'),'status'=>0);
            }
        }else{
            return  array('data'=>'','info'=>lang('OSS_NOT_SET_HTTP_CONTENT'),'status'=>0);
        }
    ...
    public function upload_file_by_file($bucket, $object, $file, $options = NULL){
        ...
        //file
        if(empty($file)){
            return  array('data'=>'','info'=>lang('OSS_FILE_PATH_IS_NOT_ALLOWED_EMPTY'),'status'=>0);
        }
        ...
        if(!file_exists($options[self::OSS_FILE_UPLOAD])){
            return  array('data'=>'','info'=>$options[self::OSS_FILE_UPLOAD].lang('OSS_FILE_NOT_EXIST'),'status'=>0);
        }
    ...
    public function create_object_group($bucket, $object_group, $object_arry, $options = NULL){
        ...
        if(empty($object_group)){
            return  array('data'=>'','info'=>lang('OSS_OBJECT_GROUP_IS_NOT_ALLOWED_EMPTY'),'status'=>0);
        }
...
function lang($key){
    $lang['NOT_SET_OSS_ACCESS_ID']			                = "未设置OSS服务的ACCESS ID";
    $lang['NOT_SET_OSS_ACCESS_KEY']			                = "未设置OSS服务的ACCESS KEY";
    $lang['NOT_SET_OSS_ACCESS_ID_AND_ACCESS_KEY']			= "没有设置ACCESS ID & ACCESS KEY";
    $lang['OSS_ACCESS_ID_OR_ACCESS_KEY_EMPTY']			    = "ACCESS ID或ACCESS KEY为空";
    $lang['OSS_LANG_FILE_NOT_EXIST']			            = "OSS语言包文件不存在";
    $lang['OSS_CONFIG_FILE_NOT_EXIST']			            = "conf.inc.php不存在";
    $lang['OSS_UTILS_FILE_NOT_EXIST']			            = "utils.php不存在";
    $lang['OSS_CURL_EXTENSION_MUST_BE_LOAD']			    = "系统没有安装CURL扩展";
    $lang['OSS_NO_ANY_EXTENSIONS_LOADED']			        = "系统没有安装任何扩展,请检查系统配置";
    $lang['OSS_WRITE_LOG_TO_FILE_FAILED']			        = "日志写入失败,请检查日志文件是否存在或者日志文件的权限";
    $lang['OSS_LOG_PATH_NOT_EXIST']			                = "日志路径不存在";
    $lang['OSS_OPTIONS_MUST_BE_ARRAY']		            	= "option必须为数组";
    $lang['OSS_GET_BUCKET_LIST_SUCCESS']	        		= "获取Bucket列表成功";
    $lang['OSS_GET_BUCKET_LIST_FAILED']			          = "获取Bucket列表失败";
    $lang['OSS_CREATE_BUCKET_SUCCESS']			          = "创建Bucket成功";
    $lang['OSS_CREATE_BUCKET_FAILED']			          = "创建Bucket失败";
    $lang['OSS_DELETE_BUCKET_SUCCESS']			          = "删除Bucket成功";
    $lang['OSS_DELETE_BUCKET_FAILED']			          = "删除Bucket失败";
    $lang['OSS_BUCKET_NAME_INVALID']			          = "未通过Bucket名称规则校验";
    $lang['OSS_GET_BUCKET_ACL_SUCCESS']			          = "获取Bucket ACL成功";
    $lang['OSS_GET_BUCKET_ACL_FAILED']			          = "获取Bucket ACL失败";
    $lang['OSS_SET_BUCKET_ACL_SUCCESS']			          = "设置Bucket ACL成功";
    $lang['OSS_SET_BUCKET_ACL_FAILED']			          = "设置Bucket ACL失败";
    $lang['OSS_ACL_INVALID']			                      = "ACL不在允许范围,目前仅允许(private,public-read,public-read-write三种权限)";
    $lang['OSS_BUCKET_IS_NOT_ALLOWED_EMPTY']		      = "Bucket不允许为空";
    $lang['OSS_TARGET_BUCKET_IS_NOT_ALLOWED_EMPTY']			= "TargetBucket不允许为空";
    $lang['OSS_INDEX_DOCUMENT_IS_NOT_ALLOWED_EMPTY']			= "IndexDocument不允许为空";
    $lang['OSS_GET_OBJECT_LIST_SUCCESS']			= "获得OBJECT列表成功";
    $lang['OSS_GET_OBJECT_LIST_FAILED']			    = "获得OBJECT列表失败";
    $lang['OSS_CREATE_OBJECT_DIR_SUCCESS']			= "创建OBJECT目录成功";
    $lang['OSS_CREATE_OBJECT_DIR_FAILED']			= "创建OBJECT目录失败";
    $lang['OSS_DELETE_OBJECT_SUCCESS']			    = "删除OBJECT成功";
    $lang['OSS_DELETE_OBJECT_FAILED']			    = "删除OBJECT失败";
    $lang['OSS_UPLOAD_FILE_BY_CONTENT_SUCCESS']			= "通过Http Body上传文件成功";
    $lang['OSS_UPLOAD_FILE_BY_CONTENT_FAILED']			= "通过Http Body上传文件失败";
    $lang['OSS_GET_OBJECT_META_SUCCESS']			= "获得OBJECT META成功";
    $lang['OSS_GET_OBJECT_META_FAILED']			    = "获得OBJECT META失败";
    $lang['OSS_OBJECT_NAME_INVALID']			    = "未通过Object名称规则校验";
    $lang['OSS_OBJECT_IS_NOT_ALLOWED_EMPTY']			= "Object不允许为空";
    $lang['OSS_INVALID_HTTP_BODY_CONTENT']			= "Http Body的内容非法";
    $lang['OSS_GET_OBJECT_SUCCESS']			        = "获得Object成功";
    $lang['OSS_GET_OBJECT_FAILED']			       = "获得Object失败";
    $lang['OSS_OBJECT_EXIST']			           = "Object存在";
    $lang['OSS_OBJECT_NOT_EXIST']			       = "Object不存在";
    $lang['OSS_NOT_SET_HTTP_CONTENT']			    = "为设置Http Body";
    $lang['OSS_INVALID_CONTENT_LENGTH']		    	= "非法的Content-Length值";
    $lang['OSS_CONTENT_LENGTH_MUST_MORE_THAN_ZERO']			= "Content-Length必须大于0";
    $lang['OSS_UPLOAD_FILE_NOT_EXIST']			    = "上传文件不存在";
    $lang['OSS_COPY_OBJECT_SUCCESS']			    = "拷贝Object成功";
    $lang['OSS_COPY_OBJECT_FAILED']			         = "拷贝Object失败";
    $lang['OSS_FILE_NOT_EXIST']			             = "文件不存在";
    $lang['OSS_FILE_PATH_IS_NOT_ALLOWED_EMPTY']			= "上传文件路径为空";
    $lang['OSS_CREATE_OBJECT_GROUP_SUCCESS']			= "创建Object Group成功";
    $lang['OSS_CREATE_OBJECT_GROUP_FAILED']			= "创建Object Group失败";
    $lang['OSS_GET_OBJECT_GROUP_SUCCESS']			= "获取Object Group成功";
    $lang['OSS_GET_OBJECT_GROUP_FAILED']			= "获取Object Group失败";
    $lang['OSS_GET_OBJECT_GROUP_INDEX_SUCCESS']			= "获取Object Group Index成功";
    $lang['OSS_GET_OBJECT_GROUP_INDEX_FAILED']			= "获取Object Group Index失败";
    $lang['OSS_GET_OBJECT_GROUP_META_SUCCESS']			= "获取Object Group Group Meta成功";
    $lang['OSS_GET_OBJECT_GROUP_META_FAILED']			= "获取Object Group Group Meta失败";
    $lang['OSS_DELETE_OBJECT_GROUP_SUCCESS']			= "删除Object Group Group成功";
    $lang['OSS_DELETE_OBJECT_GROUP_FAILED']			= "删除Object Group Group失败";
    $lang['OSS_OBJECT_GROUP_IS_NOT_ALLOWED_EMPTY']			= "Object Group不允许为空";
    $lang['OSS_OBJECT_ARRAY_IS_EMPTY']			= "创建Object Group的Object不允许为空";
    $lang['OSS_OBJECT_GROUP_TOO_MANY_OBJECT']			= "每个Object Group最多包含1000个Object";
    $lang['OSS_INITIATE_MULTI_PART_SUCCESS']			= "初始化Multi-Part Upload成功";
    $lang['OSS_INITIATE_MULTI_PART_FAILED']			= "初始化Multi-Part Upload失败";
    $lang['OSS_INVALID_OPTION_HEADERS']			= "OPTIONS不是数组";
