...
var wx_share = {
    conf:{
        ...
        ghid :'gh_90ba2364f6aa',
        appid :'wx43bcf839ae10dab4',
        ticket:''
    },
    //获取图片绝对地址
    _imgsrc : function(){
        var img_src  =$('img.share:eq(0)').attr('src') ;
        if(typeof img_src == "undefined"){
            $('img').each(function(i,n){
                if(/^(http|https):*/i.test($(n).attr('src'))){
                    img_src = $(n).attr('src');
                    return false;
                }
            });
    ...
    init: function(){
        this.conf.img = this.conf.img || this._imgsrc();
        if(wx_share.conf.ticket && typeof wx_share.conf.ticket == "object"){
            wx.config({
                debug: false, // 开启调试模式,调用的所有api的返回值会在客户端alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印。
                appId: wx_share.conf.appid, // 必填，公众号的唯一标识
                timestamp: wx_share.conf.ticket.timestamp, // 必填，生成签名的时间戳
                nonceStr: wx_share.conf.ticket.noncestr, // 必填，生成签名的随机串
                signature: wx_share.conf.ticket.signature,// 必填，签名，见附录1
                jsApiList: wx_share._jsApiList// 必填，需要使用的JS接口列表，所有JS接口列表见附录2
            });
        }else{
            if(typeof wx_share.conf.ticket == "string"){
                $.ajax({
                    async :false,
                    url : wx_share.conf.ticket,
                    data :{
                        'ghid' : this.conf.ghid,
    ...
    shareFriends : function(json){
        wx.onMenuShareTimeline({
            ...
            trigger: function (res) {
                json.trigger('Timeline');
            },
            success: function (res) {
                json.success('Timeline');
            },
            cancel: function (res) {
                json.cancel('Timeline');
            }
        });
    },
    //分享给朋友
    shareFriend: function(json){
        wx.onMenuShareAppMessage({
            title:wx_share.conf.title, // 分享标题
            desc: wx_share.conf.desc, // 分享描述
            link: wx_share.conf.url, // 分享链接
            imgUrl: wx_share.conf.img, // 分享图标
            trigger: function (res) {
                json.trigger('Message');
            },
            success: function (res) {
    ...
    share :function(json){
        this.init();
        wx.ready(function(){
            wx_share.shareFriends(json);
            wx_share.shareFriend(json);
            wx_share.shareQQ(json);
            wx_share.shareWeibo(json);
        });
    },
    //-------------------------------------------------------------------------------------------------------
