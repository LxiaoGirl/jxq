...
class CI_Ossupload {
        ...
        else
        {
...

            {
                ...
                foreach ($_FILES[$field]['name'] as $k => $v)
                {
                    if ( ! empty($v))
                    {
                        $query = $this->_do_xupload($field, $k);

                        if( ! empty($query))
                        {
                            $this->data[] = $this->_data();
                            $this->errdata[] = '文件'.($k+1).'. 上传成功';
                        }
                        else
                        {
                            $success=FALSE;
                            $this->errdata[] = '文件'.($k+1).'. 上传失败 ERROR:'.array_pop($this->error_msg);
                        }
                    }
                }
     ...
     * @return	bool
     */
    private function _do_upload($field = 'userfile')
    {
...
//        }
        $ci=&get_instance();
        $ci->load->library('oss',array('access_id'=>$ci->config->item('oss_access_id'),'access_key'=>$ci->config->item('oss_access_key')));
        $result=$ci->oss->upload_file_by_file($ci->config->item('oss_bucket_img'),$this->upload_path.$this->file_name,$this->file_temp);
        if($result['status'] == 1){
            if($result['data']->status != 200){
                $dom=new DomDocument;
                $dom->loadXML($result['data']->body);
                $this->set_error($dom->getElementsByTagName('Error')->item(0)->getElementsByTagName('Message')->item(0)->nodeValue);
                return FALSE;
            }
        }else{
            $this->set_error($result['info']);
            return FALSE;
        }
    ...
    private function _do_xupload($field = 'userfile', $index = 0)
    {
        // Is $_FILES[$field] set? If not, no reason to continue.
        if ( ! isset($_FILES[$field]))
        {
...
//        }
    ...
    {
        $str = '';
        if( ! empty($this->errdata)){
            $errdata=$this->errdata;
        }else{
            $errdata=$this->error_msg;
        }
