            ...
            if( ! empty($temp['user'])){
                ...
                if($temp['minute'] == 0){
                    ...
                    if($temp['user']['password'] == $temp['password']){
                        ...
                        $this->_add_user_log('login', 'mobiles会员登录', $temp['user']['uid'], $temp['user']['user_name']);
                        $data = array(
                            'status' => 0,
                            'msg'  => '欢迎您的光临！',
                            'data'  => array()
...
/*******************************************设置相关**********************************************/
    ...
    public function ajax_forget_check(){
        ...
        $temp['is_check'] = $this->send->validation($temp['mobile'], $temp['code'], 2, 5);

        if( ! empty($temp['is_check'])){
            $data = array('status' => 0, 'msg' => '请输入新密码！', 'data' => '');
        }else{
