...
class Common_model extends CI_Model
{
    ...
    public function create_links($total = 0, $page = 10)
    {
        ...
        $config['num_links']            = 2;
        $config['full_tag_open']        = '<div class="u-page">';
        $config['full_tag_close']       = '</div>';
        $config['cur_tag_open']         = '<a href="javascript:void(0);" class="z-sel">';
    ...
    public function set($table = '', $where = array(), $user_data = array())
    {
        ...
        if( ! empty($table) && ! empty($where) && isset($user_data['field']) && isset($user_data['value']))
        {
            $this->db->set($user_data['field'], $user_data['value'], FALSE);
            $this->_where($where);
    ...
    private function _where($where = array())
    {
        ...
        if (isset($where['like']) && is_array($where['like']))
        {
            ...
            else
            {
                ...
                {
                    if(isset($where['like']['flag']) && in_array($where['like']['flag'], array('before', 'after', 'both')))
                    {
...
/**************************************************************web 2.0 新增 2015-11-03 wsb**************************************************************************************************/
	...
	public function get_category(){
		...
		$category = $this->project->get_project_category();
		if($category['status'] == '10000'){
	 ...
	 * @return mixed
	 */
	public function get_links($total = 0,$page_id=1, $page_size = 10){
		$this->load->library('pagination_1');

		$config['base_url']             = $this->show_url(TRUE);
		$config['total_rows']           = (int)$total;
		$config['per_page']             = (int)$page_size;
		$config['num_links']            = 3;
		$config['full_tag_open']        = '<div class="invest_home_paging tc">';
		$config['full_tag_close']       = '</div>';
		$config['cur_tag_open']         = '<font class="active">';
		...
		$config['query_string_segment'] = 'per_page';
		$config['tag'] = 'font';
		$config['cur_page'] = (int)(((int)$page_id-1)*(int)$page_size);

		$this->pagination_1->initialize($config);
		return $this->pagination_1->create_links();
	}
	 ...
	 * @return float
	 */
	public function get_page_id($page_size=10){
		if(isset($_GET['per_page'])){
			return (int)$this->input->get('per_page',true)/$page_size + 1;
		}else{
            return 1;
        }
	}
