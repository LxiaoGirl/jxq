...
class Sms_model extends CI_Model{
    ...
    protected $_msg = array(
        'default'=>'服务器繁忙,请稍后重试或联系客服！',
        'mobiles_format_error'=>'手机号码格式不正确！',
        'sms_content_empty'=>'短信内容不能为空！'
    );

    protected $_api_name_array = array(
        'send_sms'=>''
    );
    ...
    public function send_sms($mobile = '', $content = '', $type = 1, $uid = 0){
        $data = array(
            'api_name'=>$this->_api_name_array[__METHOD__],
            'status' => 0,
            'msg' => $this->_msg['default'],
            'data'=>array()
        );
        $temp = array();
        ...
        if( ! $this->Regex->is_mobile($mobile)){
            $data['msg'] = $this->_msg['mobiles_format_error'];
            return  $data;
        }

        //验证发送内容
        if($content == ''){
            $data['msg'] = $this->_msg['sms_content_empty'];
            return  $data;
        }
        //验证ip target 次数
	...
	public function voice($mobile = '', $content = '', $type = 1, $uid = 0){
        ...
        $temp['code']    = $this->_get_random($mobile, 6);
        $temp['content'] = sprintf($content, $temp['code']);
        $query = $this->_voiceVerify($temp['code'],"3",$mobile,self::VOICE_TEL,"");
     ...
     * @return bool
     */
	protected function _voiceVerify($verifyCode,$playTimes,$to,$displayNum,$respUrl){
		// 初始化REST SDK
        $rest = new REST();
        //调用语音验证码接口
...
/*表结构：
CREATE TABLE IF NOT EXISTS `cdb_authcode` (
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC AUTO_INCREMENT=113314 ;*/