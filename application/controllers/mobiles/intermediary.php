...
class Intermediary extends MY_Controller{
    ...
    public function __construct(){
        ...
        //验证居间人起始时间
        if($this->uri->uri_string() != 'mobiles/intermediary/no_start')$this->_check_intermediary_time();
    ...
    public function guide(){
        ...
        $this->load->model('mobiles/intermediary_model','intermediary');
        $data['redbag'] = $this->intermediary->get_redbag(20);

        $_GET['limit'] = 20;
        $data ['invest_list'] = $this->c->show_page(self::payment,
            array(
                'where'=>array(join_field('type',self::payment)=>1,join_field('status',self::payment)=>1),
                'order_by'=>join_field('dateline',self::payment).' DESC',
                'select'=>join_field('mobile',self::user).','.join_field('dateline,amount',self::payment),
                'join'=>array(
                    'table'=>self::user,
...
//        $data['uid'] = authcode((int)$this->input->get('uid'),'',TRUE);
    ...
    public function share_weixin(){
        ...
        if($this->_is_weixin()){
            ...
            $this->session->set_userdata(array('is_myself'=>1));//用于 验证是不是自己打开的分享页面  如果是自己 则加上分享引导图层
            redirect('/mobiles/intermediary/share_page?inviter_no='.$this->session->userdata('inviter_no'));
    ...
    public function real_name(){
        if($this->input->is_ajax_request() == TRUE){
            $this->load->model('web_1/user_model','user');
