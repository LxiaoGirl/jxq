    ...
    $(function () {
        ...
        $("#btn_tj").click(function (event) {
            ...
            if (!card_no) {
                my_alert('请先绑定银行卡！', 2);
                setTimeout(function () {
                    window.location.replace('<?php echo site_url('apps/home/my_card') ?>');
                }, 2000);
                return false;
            }
        ...
        $(".ok").on('tap', function () {
            ...
            $.ajax({
                url: '/index.php/apps/home/transfer',
                dataType: 'json',
                type: 'post',
                data: {
                    'card_no': card_no,
                ...
                success: function (resut) {
                    if (resut.code == 0) {
                        window.location.replace('<?php echo site_url('apps/home/transfer_success?amount='); ?>' + $("input[name='amount']").val());
                    } else {
                        my_alert(resut.msg);
                    }
                }
