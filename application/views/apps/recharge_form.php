...
<div class="container-fluid" style="">
    ...
    <!-- 顶部红色部分 end-->
    <form action="<?php echo site_url('apps/home/recharge_confirm'); ?>" role="form" onsubmit="return check_form();"
          method="post">
        <div class="row  tonglan_input  form-group-lg">
            <p class="ml10 mt10">
                开户银行：<a href="<?php echo site_url('apps/home/recharge_notice'); ?>" style="color:#ff8700;"><font
...
<script>
    ...
    if('<?php echo $card?$card['account']:''; ?>'){
        ...
        $("input[name='account']").bind('blur',function(){
            ...
            $.post('<?php echo site_url('apps/home/ajax_check_card_bin'); ?>', {'account': $("input[name='account']").val()}, function (rs) {
                if (rs.ret_code == '0000') {
                    if (rs.card_type == '3') {
    ...
    var check_form = function () {
        ...
        if (!amount_reg.test($("input[name='amount']").val())) {  //
            my_alert('请输入0元以上充值金额！');
            return false;
        }
        if (!/^[0-9]*$/.test($("input[name='amount']").val())) {  //
            my_alert('请输入不小于10的整数金额！');
            return false;
        }
