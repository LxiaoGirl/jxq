            ...
            <div class="row  tonglan_input  form-group-lg">
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <a href="<?php echo site_url('apps/home/recharge'); ?>" class="btn btn-lg btn-info">去充值</a>
                        </td>
                    </tr>
                    </tbody>
                </table>
...
<script>
    ...
    var myInvest = function (mon) {
        if (isLogin) {
            if (security == '0') {
                ...
                setTimeout(function () {
                    window.location.href = '<?php echo site_url('apps/home/security');?>';
                }, 2000);
                return;
            }
            var money = $("#" + mon).val();
    ...
    var check_secrity = function (mon, borrow_no) {
        ...
        if (secu != '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo site_url('apps/home/ajax_invest') ?>',
                data: {amount: money, borrow_no: borrow_no, password: secu},
                dataType: 'json',
                success: function (result) {
                    $("#btn_toubiao").prop('disabled', false).text('投标');
                    if (result.status == '10000') {
                        window.location.replace('<?php echo site_url('apps/home/project_invest_success') ?>');
                    } else {
                        my_alert(result.msg);
                    }
                }
