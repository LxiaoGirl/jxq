        ...
        <div role="form">
            ...
            <div class="container">
                <div class="row mt10 mb20">
                    <button id="submit" class="btn btn-lg btn-danger btn-block ajax-submit-button">确认修改</button>
                </div>
            </div>
        </div>
...
<script>
    ...
    var changPW = function () {
        ...
        $.post('<?php echo site_url('apps/home/password') ?>', {
            'password': $("input[name='password']").val(),
            'new_password': $("input[name='new_password']").val(),
            'authcode': $("input[name='authcode']").val()
        }, function (rs) {
            if (rs.code == 0) {
                window.location.replace('<?php echo site_url('apps/home/password_success') ?>');
            } else {
                //关闭 弹窗 然后提示
                my_alert(rs.msg);
            }
