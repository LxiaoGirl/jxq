    ...
    <form id="myform" action="" method="post" onsubmit="return false;">
       ...
       <div class="box">
        ...
        <div class="box" style="display:none">
            <input type="checkbox" id="agree" checked style="    height: auto;width: auto;border: none;padding-left: 0;border: 0;"><a style="color:#639BFD;" href="<?php echo site_url('apps/intermediary/agree'); ?>">申请协议</a>
            <p class="agree"></p>
        </div>
...
$(function(){
    ...
    var check_phone = function(){
        if($('input[name="mobile"]').val()==""){
            ...
            $.ajax({
                type: 'POST',
                url: '/index.php/apps/intermediary/ajax_intermediary_check',
    ...
    $("#yuyin").keyup(function(){
        if($(this).val()==""){
            ...
            $.ajax({
                type: 'POST',
                url: '/index.php/apps/intermediary/ajax_authcode_check',
    ...
    $("#tijiao2").click(function(){
        if(chkusername==true&&chkcode==true&&chkvoice==true){
            mobiel = $('input[name="mobile"]').val();
            authcode = $("#yuyin").val();
            if(isLogin){
                $("#tijiao2").html('提交中...');
                button2_status(false);
                $.post('/index.php/apps/intermediary/ajax_intermediary_apply',{'mobile':mobiel,'password':'','authcode':authcode,'source':'<?php echo $inviter_no; ?>'},function(rs){
                    $("#tijiao2").html('下一步');
                    button2_status(true);
                    if(rs.code == 0){
    ...
    $("#pass_btn").click(function(){
        if(chkpwd==true&&chkcpwd==true){
            button3_status(false);
            $("#pass_btn").html('提交中...');
            $.post('/index.php/apps/intermediary/ajax_intermediary_apply',{'mobile':mobiel,'password':$("#pwd").val(),'authcode':authcode,'source':'<?php echo $inviter_no; ?>'},function(rs){
                button3_status(true);
                $("#pass_btn").html('确定');
                if(rs.code == 0){
                    window.location.replace(rs.data);
