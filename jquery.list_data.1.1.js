...
(function($){
    ...
    var list_data = function(option){
        ...
        //验证是单循环还是多循环 处理html数据结构
        if( this.option.list_one){
            this.option.is_scroll?scroll_html_init():this.html_data.loading = this.option.id.find('#loading').length?this.option.id.find('#loading').clone().remove():$('<span>'+this.temp_data.msg_loading+'</span>');
        }else{
            //在 非滑动时 获取 循环主题 没有数据 没有更多数据 加载中时 更多的button 的 html数据
            if( ! this.option.is_scroll){
                this.html_data.no_data = this.option.id.find('#no-data').length?this.option.id.find('#no-data').clone().remove():false;
                this.html_data.no_more = this.option.id.find('#no-more').length?this.option.id.find('#no-data').clone().remove():false;
                this.html_data.loading = this.option.id.find('#loading').length?this.option.id.find('#no-data').clone().remove():false;
                this.html_data.more_button = this.option.id.find('#more-button').length?this.option.id.find('#no-data').clone().remove():false;
            }
                            ...
                            if(this.option.list_fadein > 0  && this.temp_data.list_fadein_time > 0){
                                var list_fadein_t = setTimeout(function(){
                                    ...
                                    $(window).bind('scroll',function(){
                                        if($(this).scrollTop() + $(this).height() == $(document).height())that._init();
                                    });
                                },this.temp_data.list_fadein_time);
                            }else{
                                //滑动到底部时的加载事件处理
...
function ajax_file_upload(id,name){
    var my_file = document.getElementById(id).files[0];
    var fd = new FormData();
    fd.append(name,my_file);
...
//                son.style.width=per+"%";
    ...
    xhr.open("post","/index.php/login/company_register");
    xhr.send(fd);
}
function ajax_upload_file(id,url,func){
    ...
    function onprogress(evt){
        ...
        if(typeof func == "function")func(per,false);
    }
    /**
    ...
    $.ajax({
        ...
        success:function(rs){
            if(typeof func == "function")func(per,rs);
        }
    });
}
